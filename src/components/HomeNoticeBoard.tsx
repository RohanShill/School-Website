  ...
  return (
    <section className="py-12 sm:py-16 bg-slate-50 border-y border-slate-200">
      <div className="section-shell">
        {/* Section Header */}
        <div className="mb-8 sm:mb-10 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <h2 className="mt-3 text-2xl sm:text-3xl lg:text-4xl font-black text-brand-navy leading-snug">
        ...
        <div className="grid gap-6 lg:gap-8 lg:grid-cols-[1.4fr_0.8fr]">
          ...
          <div className="rounded-2xl sm:rounded-3xl border border-slate-200 bg-white p-4 sm:p-6 shadow-soft">
            ...
            <div className="mb-5 flex overflow-x-auto no-scrollbar gap-2 border-b border-slate-100 pb-3 -mx-2 px-2 sm:mx-0 sm:px-0 sm:flex-wrap">
              {categories.map((cat) => (
                <button
                  key={cat}
                  type="button"
            ...
            <div className="space-y-3.5 sm:space-y-4">
              {filteredNotices.length === 0 ? (
                ...
                filteredNotices.map((notice) => (
                  <div
                    key={notice.id}
                    className="group relative rounded-xl sm:rounded-2xl border border-slate-100 bg-slate-50/70 p-3.5 sm:p-4 transition-all duration-200 hover:border-brand-blue/30 hover:bg-white hover:shadow-md"
                  >
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2.5 sm:gap-3">
                      <div className="space-y-1 flex-1">
                        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                          <span className="rounded-full bg-brand-navy/10 px-2 py-0.5 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider text-brand-navy">
                            {notice.category}
                          </span>
                          {notice.isUrgent && (
                            <span className="inline-flex items-center gap-1 rounded-full bg-red-100 px-2 py-0.5 text-[9px] sm:text-[10px] font-bold text-red-600 animate-pulse">
                        ...
                        <h3 className="text-sm sm:text-base font-bold text-brand-navy group-hover:text-brand-blue transition leading-snug">
                          {notice.title}
                        </h3>
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0 pt-1 sm:pt-0 self-end sm:self-auto">
                        <a
                          href={notice.pdfUrl || '#'}
                          className="inline-flex items-center gap-1.5 rounded-lg sm:rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-brand-navy shadow-2xs hover:bg-brand-blue hover:text-white transition active:scale-95"
                        >
                          <Download size={13} />
                          <span>PDF / View</span>
                        </a>
          ...
          <div className="flex flex-col gap-4 sm:gap-6">
            ...
            <div className="rounded-2xl sm:rounded-3xl border border-slate-200 bg-white p-4 sm:p-6 shadow-soft">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3.5 mb-3.5">
                <div className="flex items-center gap-2.5">
                  <div className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-xl bg-brand-gold/20 text-brand-navy">
                    <Calendar size={16} />
                  </div>
                  <div>
              ...
              <div className="space-y-2.5">
                {upcomingEvents.map((evt, index) => (
                  <div
                    key={index}
                    className="flex items-start gap-2.5 rounded-xl border border-slate-100 bg-slate-50 p-2.5 hover:bg-slate-100/80 transition"
                  >
                    <div className="flex flex-col items-center justify-center rounded-lg bg-white border border-slate-200 px-1.5 py-1 text-center min-w-[44px]">
                      <span className="text-[9px] font-bold text-brand-blue uppercase">{evt.tag}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="text-xs font-bold text-brand-navy leading-tight">
                        {language === 'hi' ? evt.title : evt.titleEn}
                      </h4>
                      <p className="text-[10px] text-slate-500 mt-0.5">{evt.date}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Quick Helpline Box */}
            <div className="rounded-2xl sm:rounded-3xl bg-gradient-to-br from-brand-navy to-slate-900 p-4 sm:p-6 text-white shadow-soft">
              ...
              <div className="mt-3.5 pt-3.5 border-t border-white/10 flex items-center justify-between">
                <span className="text-[11px] text-brand-gold font-semibold">UDISE: 20120104801</span>
                <Link
                  href="/admissions"
                  className="rounded-full bg-brand-gold px-3.5 py-1.5 text-xs font-bold text-brand-navy hover:bg-yellow-400 active:scale-95 transition"
                >
